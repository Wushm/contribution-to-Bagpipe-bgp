...
.. module:: test_vpn_instance
   ...
   - testEx use cases to test if instanceLabel can well take the value of vnid
...
class TestVPNInstance(TestCase):
    ...
    def setUp(self):
        ...
        self.vpnInstance = TestableVPNInstance(mock.Mock(name='BGPManager'),
                                               self.labelAllocator,VNID1,
                                               self.mockDPDriver, 1, 1,
                                               [RT1], [RT1], '10.0.0.1', 24,
                                               None)
        self.vpnInstance1 = TestableVPNInstance(mock.Mock(name='BGPManager'),
                                               self.labelAllocator,VNID2,
                                               self.mockDPDriver, 1, 1,
                                               [RT1], [RT1], '10.0.0.1', 24,
                                               None)
    ...
    def testE1_VNIDisVideOnRESTAPI(self):
	'''
	the VNID is None on REST api, to test if the vnid generated by labelAllocator is well taken by self.instanceLabel
	'''
	labelInstance = self.vpnInstance.instanceLabel
	self.assertNotEqual(VNID1,labelInstance)

    def testE2_VNIDisPassedByRESTAPT(self):
	'''
	the VNID is passed by REST API, to test if this vnid is well taken by the self.instanceLabel
	'''
	labelInstance = self.vpnInstance1.instanceLabel 
	self.assertEqual(VNID2,labelInstance)
