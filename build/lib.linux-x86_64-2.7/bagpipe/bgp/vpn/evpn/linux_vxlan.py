...
class LinuxVXLANEVIDataplane(VPNInstanceDataplane):
	...
	vxlanProxySpec = ""
	if self.driver.vxlanProxy:
	    vxlanProxySpec = " %s " % self.driver.vxlanProxy

        # Create VXLAN interface
        self._runCommand(
            "ip link add %s type vxlan id %d local %s nolearning%s%s" %
            (self.vxlan_if_name, self.instanceLabel,
             self.driver.getLocalAddress(), vxlanProxySpec, dstPortSpec)
        )
...
class LinuxVXLANDataplaneDriver(DataplaneDriver):
    ...
    def __init__(self, config, init=True):
        ...
        try:
	    self.vxlanProxy = config.get("vxlan_proxy","" )
	except ValueError:
	    raise Exception("Could not parse vxlan ARP proxy situation:%s" %
			     config["vxlan_proxy"]) 
